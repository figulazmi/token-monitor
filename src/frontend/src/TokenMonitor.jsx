...
function calcCost(log) {
  ...
  if (!model) return 0;
  return (
    (log.inputTokens / 1_000_000) * model.inputRate +
...
export default function TokenMonitor() {
  ...
  async function handleAssignAccount(id, account) {
    try {
      ...
      setSessions((prev) =>
        prev.map((s) => (s.id === id ? { ...s, account } : s)),
      );
      setAssigningId(null);
  ...
  sessions.forEach((l) => {
    hourBuckets[new Date(l.ts).getHours()] += l.inputTokens + l.outputTokens;
  });
  ...
  const barMax = Math.max(
    ...sessions.map((l) => l.inputTokens + l.outputTokens),
    1,
  );
    ...
    >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div
              style={{
                ...
                height: 36,
                borderRadius: 8,
                background: 'linear-gradient(135deg,#FF6B35,#0078D4)',
                display: 'flex',
                alignItems: 'center',
            ...
            <div>
              ...
              <div
                style={{
                  fontSize: 10,
                  color: apiError ? '#cc4444' : '#555',
                  letterSpacing: '0.1em',
                }}
              >
                {apiError
                  ? '⚠ API OFFLINE'
                  : 'CLAUDE CODE · COPILOT · 3 ACCOUNTS'}
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              style={{
                background: showForm
                  ? '#1E1E2E'
                  : 'linear-gradient(135deg,#FF6B35,#c94b1a)',
                border: 'none',
                borderRadius: 6,
                color: '#fff',
                padding: '8px 16px',
      ...
      <div style={{ padding: '24px 28px', maxWidth: 960, margin: '0 auto' }}>
        ...
        {apiError && !loading && (
          ...
          >
            ⚠ Cannot connect to API at <code>{API_URL}</code>. Make sure the
            backend is running.
            <button
              onClick={fetchData}
              style={{
        ...
        {showForm && (
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, inputTokens: e.target.value }))
                  }
                  style={S.input}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, outputTokens: e.target.value }))
                  }
                  style={S.input}
                />
              </div>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input
                  type='text'
                  placeholder='e.g. SDL middleware refactor'
                  value={form.label}
                  onChange={(e) =>
                ...
                <span style={{ color: '#FF6B35' }}>
                  {fmtCost(
                    calcCost({
                      ...
                      outputTokens: +form.outputTokens,
                    }),
                  )}
                </span>
              </div>
            ...
            <button
              ...
              style={{
                marginTop: 16,
                background: 'linear-gradient(135deg,#FF6B35,#c94b1a)',
                border: 'none',
                borderRadius: 6,
                color: '#fff',
                padding: '10px 24px',
          ...
          <>
            ...
            >
              ...
              >
                {(stats?.by_account || []).map((a) => {
                  const meta = ACCOUNT_META[a.account] || {
                    label: a.account,
                    color: '#888',
                  };
                  const total = stats?.total_cost_usd || 0;
                  const pct =
                    total > 0 ? Math.round((a.cost_usd / total) * 100) : 0;
                  return (
                    <div
                      key={a.account}
                      style={{
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: 10, color: '#444', marginTop: 2 }}
                      >
                        {a.sessions} sessions · {pct}%
                      </div>
                    </div>
                  );
                })}
            ...
            >
              {[
                ...
                const stat = stats?.by_platform?.find(
                  (r) => r.platform === p.key,
                );
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: 10, color: p.color }}>
                        {pct}%
                      </span>
                    </div>
                    <div
                      style={{ fontSize: 22, fontWeight: 700, color: p.color }}
                    >
                      {fmtCost(cost)}
                    </div>
                ...
                {hourBuckets.map((val, h) => (
                  <div key={h} style={{ flex: 1 }}>
                    <div
                      style={{
                        width: '100%',
                        height: `${Math.max(4, (val / maxBucket) * 44)}px`,
                        background: h === peakHour ? '#FF6B35' : '#1E1E2E',
                        borderRadius: 2,
                      }}
                    />
                  </div>
            ...
            >
              {['all', 'claude', 'copilot'].map((f) => (
                <button
                  ...
                  onClick={() => setFilter(f)}
                  style={{
                    background: filter === f ? '#1E1E2E' : 'none',
                    border: `1px solid ${filter === f ? '#FF6B35' : '#1E1E2E'}`,
                    borderRadius: 6,
                    color: filter === f ? '#FF6B35' : '#444',
                    padding: '6px 14px',
                    fontSize: 10,
                    cursor: 'pointer',
                    letterSpacing: '0.08em',
              ...
              {unassignedCount > 0 && (
                <button
                  ...
                  style={{
                    background: filter === 'unassigned' ? '#2A1F00' : 'none',
                    border: `1px solid ${filter === 'unassigned' ? '#F0A500' : '#2A2200'}`,
                    borderRadius: 6,
                    color: '#F0A500',
                    padding: '6px 14px',
                    fontSize: 10,
              ...
              {[...filtered].map((l) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: '#0D0D14',
                      border: `1px solid ${unassigned ? '#2A2200' : '#1E1E2E'}`,
                      borderRadius: 10,
                      padding: '14px 16px',
                    }}
                  >
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 8,
                          borderRadius: '50%',
                          background: unassigned ? '#F0A500' : meta.color,
                          flexShrink: 0,
                        }}
                      />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {unassigned ? (
                            <span
                              onClick={() =>
                                setAssigningId(isAssigning ? null : l.id)
                              }
                              style={{
                                color: '#F0A500',
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        ...
                        <div
                          style={{
                            fontSize: 10,
                            color: unassigned ? '#F0A500' : meta.color,
                          }}
                        >
                          {fmtCost(cost)}
                        </div>
                    ...
                    {isAssigning && (
                      ...
                      >
                        ...
                        {Object.entries(ACCOUNT_META).map(([id, m]) => (
                          <button
                            key={id}
                            onClick={() => handleAssignAccount(l.id, id)}
                            style={{
                              background: '#0A0A0F',
